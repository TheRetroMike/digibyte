...
bool Chainstate::ActivateBestChain(BlockValidationState& state, std::shared_ptr<const CBlock> pblock)
{
    ...
    do {
...

        {
            ...
            LOCK(MempoolMutex());
            // Also lock stempool if it exists, as MaybeUpdateMempoolForReorg requires it
            LOCK(StempoolMutex());
...
bool HasValidProofOfWork(const std::vector<CBlockHeader>& headers, const Consensus::Params& consensusParams)
{
    return std::all_of(headers.cbegin(), headers.cend(),
            [&](const auto& header) { return CheckProofOfWork(header.GetPoWAlgoHash(consensusParams), header.nBits, consensusParams);});
}
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // in "Weaknesses in DigiByte’s Merkle Root Construction":
        // https://lists.linuxfoundation.org/pipermail/digibyte-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
...
Chainstate& ChainstateManager::InitializeChainstate(CTxMemPool* mempool, CTxMemPool* stempool)
{
    AssertLockHeld(::cs_main);
    assert(!m_ibd_chainstate);
    assert(!m_active_chainstate);

    m_ibd_chainstate = std::make_unique<Chainstate>(mempool, stempool, m_blockman, *this);
        ...
        bool in_memory)
{
    ...
    auto snapshot_chainstate = WITH_LOCK(::cs_main,
        return std::make_unique<Chainstate>(
            /*mempool=*/nullptr, /*stempool=*/nullptr, m_blockman, *this, base_blockhash));
...
{
    // DigiByte has no historical duplicate coinbase transactions.
    // Bitcoin had two duplicate coinbase pairs at blocks 91842/91880 and 91812/91842
    // which required special exception handling. These do not exist in DigiByte's chain.
    // Always return false to ensure proper BIP30 enforcement on all networks.
    return false;
}

bool IsBIP30Unspendable(const CBlockIndex& block_index)
{
    // DigiByte has no historical duplicate coinbase transactions that were overwritten.
    // Bitcoin's blocks 91722/91812 had their coinbase outputs overwritten by duplicates.
    // This never occurred in DigiByte, so always return false.
    return false;
}
