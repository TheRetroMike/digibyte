...
BOOST_AUTO_TEST_CASE(stale_tip_peer_management)
{
    ...
    // For DigiByte: nPowTargetSpacing is 15 seconds, stale tip threshold is 3 * 15 = 45 seconds
    // We need to wait for STALE_CHECK_INTERVAL (10 minutes) plus the stale threshold
    const auto time_later{time_init + 11min + 3 * std::chrono::seconds{m_node.chainman->GetConsensus().nPowTargetSpacing} + 1s};
...
// Note: The orphanage tests have been moved to orphanage_tests.cpp in Bitcoin v26.2
// This duplicate test was causing double-locking issues and has been removed.
