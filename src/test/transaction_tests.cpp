...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    // P2PKH output dust threshold calculation:
    // Size = 34 (output) + 148 (input to spend) = 182 bytes
    // With DigiByte's dustRelayFee of 30000 sat/kvB: 182 * 30000 / 1000 = 5460
    ...
    // nDustThreshold = 182 * 3702 / 1000 = 673.764 (rounds to 674)
    ...
    t.vout[0].nValue = 5760;  // DigiByte: 10x Bitcoin value
    CheckIsStandard(t);
    t.vout[0].nValue = 5759;  // DigiByte: 10x Bitcoin value
    CheckIsNotStandard(t, "dust");
    ...
    t.vout[0].nValue = 6720;  // DigiByte: 10x Bitcoin value
    CheckIsStandard(t);
    t.vout[0].nValue = 6719;  // DigiByte: 10x Bitcoin value
    CheckIsNotStandard(t, "dust");
    ...
    t.vout[0].nValue = 5460;  // DigiByte: 10x Bitcoin value
    CheckIsStandard(t);
    t.vout[0].nValue = 5459;  // DigiByte: 10x Bitcoin value
    CheckIsNotStandard(t, "dust");

    // Check P2SH outputs dust threshold
    t.vout[0].scriptPubKey = CScript() << OP_HASH160 << std::vector<unsigned char>(20, 0) << OP_EQUAL;
    t.vout[0].nValue = 5400;  // DigiByte: 10x Bitcoin value
    CheckIsStandard(t);
    t.vout[0].nValue = 5399;  // DigiByte: 10x Bitcoin value
    CheckIsNotStandard(t, "dust");

    // Check P2WPKH outputs dust threshold
    t.vout[0].scriptPubKey = CScript() << OP_0 << std::vector<unsigned char>(20, 0);
    t.vout[0].nValue = 2940;  // DigiByte: 10x Bitcoin value
    CheckIsStandard(t);
    t.vout[0].nValue = 2939;  // DigiByte: 10x Bitcoin value
    CheckIsNotStandard(t, "dust");

    // Check P2WSH outputs dust threshold
    t.vout[0].scriptPubKey = CScript() << OP_0 << std::vector<unsigned char>(32, 0);
    t.vout[0].nValue = 3300;  // DigiByte: 10x Bitcoin value
    CheckIsStandard(t);
    t.vout[0].nValue = 3299;  // DigiByte: 10x Bitcoin value
    CheckIsNotStandard(t, "dust");
    ...
    for (int op = OP_1; op <= OP_16; op += 1) {
        ...
        t.vout[0].nValue = 2400;  // DigiByte: 10x Bitcoin value
        CheckIsStandard(t);

        t.vout[0].nValue = 2399;  // DigiByte: 10x Bitcoin value
        CheckIsNotStandard(t, "dust");
    }
}
