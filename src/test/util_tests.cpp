...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    BOOST_CHECK(!ParseMoney("21000000001.00"));  // Exceeds DigiByte's 21 billion max
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    const std::array<unsigned char, 32> privkey_bytes = {
        ...
        // derived address from this private key: D5CRxFdyRpGZLW9w8HnHvVduizdL5jKNbs
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "SefNWSMmXAxc4N8sxHyg8hHXTTU9GojgzX",
            "signature should be irrelevant",
            "message too"),
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "DSC6tGC2NvN6AHio2p8pU4seTbPKDi7KZb",
