...
/* Define a virtual block time, one block per 15 seconds after Nov 14 2014, 0:55:36am */
static int32_t TestTime(int nHeight) { return 1415926536 + 15 * nHeight; }
...
static void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(nullptr, params, ALGO_SCRYPT) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
    ...
    if (nTime == 0) {
        ...
        BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params, ALGO_SCRYPT) & (1 << bit), 0);
