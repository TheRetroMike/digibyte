...
/** DigiByte: Dandelion++ variant that allows specifying a custom mempool (for stempool) */
...
public:
    ...
    //! @returns A pointer to the stempool.
    CTxMemPool* GetStempool()
    {
        return m_stempool;
    }
    ...
    //! Indirection necessary to make lock annotations work with an optional stempool.
...
public:
    ...
    //                                  constructor
    Chainstate& InitializeChainstate(CTxMemPool* mempool, CTxMemPool* stempool) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
