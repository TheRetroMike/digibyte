# User-space, Statically Defined Tracing (USDT) for DigiByte Core

DigiByte Core includes statically defined tracepoints to allow for more
...
**Note**: DigiByte Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to DigiByte Core
...
Multiple tools can list the available tracepoints in a `digibyted` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in DigiByte Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/digibyted
stap net        outbound_message 0x0000000000107c05 /src/digibyted
stap validation block_connected  0x00000000002fb10c /src/digibyted
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in DigiByte Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/digibyted | grep NT_STAPSDT -A 4 -B 2
