...
It is not necessary to build wallet functionality to run either `digibyted` or `digibyte-qt`.
...
...
to: /path/to/digibyte/depends/x86_64-unknown-openbsd
```

Then set `BDB_PREFIX`:

```bash
export BDB_PREFIX="/path/to/digibyte/depends/x86_64-unknown-openbsd"
```

#### GUI Dependencies
###### Qt5

DigiByte Core includes a GUI built with the cross-platform Qt Framework. To compile the GUI, Qt 5 is required.
