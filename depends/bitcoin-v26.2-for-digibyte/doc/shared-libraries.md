...
`digibyteconsensus_version` returns an `unsigned int` with the API version *(currently `2`)*.

#### Script Validation

`digibyteconsensus_verify_script`, `digibyteconsensus_verify_script_with_amount` and `digibyteconsensus_verify_script_with_spent_outputs` return an `int` with the status of the verification. It will be `1` if the input script correctly spends the previous output `scriptPubKey`.

##### Parameters
###### digibyteconsensus_verify_script
...
###### digibyteconsensus_verify_script_with_amount
- `const unsigned char *scriptPubKey` - The previous output script that encumbers spending.
- `unsigned int scriptPubKeyLen` - The number of bytes for the `scriptPubKey`.
...
###### digibyteconsensus_verify_script_with_spent_outputs
- `const unsigned char *scriptPubKey` - The previous output script that encumbers spending.
- `unsigned int scriptPubKeyLen` - The number of bytes for the `scriptPubKey`.
- `int64_t amount` - The amount spent in the input
- `const unsigned char *txTo` - The transaction with the input that is spending the previous output.
...
- `digibyteconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` - Enable TAPROOT ([BIP340](https://github.com/digibyte/bips/blob/master/bip-0340.mediawiki), [BIP341](https://github.com/digibyte/bips/blob/master/bip-0341.mediawiki), [BIP342](https://github.com/digibyte/bips/blob/master/bip-0342.mediawiki))
...
- `digibyteconsensus_ERR_SPENT_OUTPUTS_REQUIRED` - Spent outputs are required if TAPROOT is used
- `digibyteconsensus_ERR_SPENT_OUTPUTS_MISMATCH` - Spent outputs size doesn't match tx inputs size
...
- [rust-digibyteconsensus](https://github.com/rust-digibyte/rust-digibyteconsensus) (Rust Bindings)