# CJDNS support in DigiByte Core

It is possible to run DigiByte Core over CJDNS, an encrypted IPv6 network that
...
done outside of DigiByte Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the DigiByte network and individual nodes.
...
before it will work with your DigiByte Core node. This is described in steps
...
## Run DigiByte Core with CJDNS

Once you are connected to the CJDNS network, the following DigiByte Core
...
When enabled, this option tells DigiByte Core that it is running in an
...
IPv6 local network. This helps DigiByte Core perform better address management:
...
CJDNS support was added to DigiByte Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `digibyte-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/digibyte/digibyte/blob/master/doc/tor.md) for
details.

## CJDNS-related information in DigiByte Core

There are several ways to see your CJDNS address in DigiByte Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `digibyte-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `digibyte-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `digibyte-cli help getnodeaddresses` for details.
