...
/** Call node.updateRwSetting() with DigiByte 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because DigiByte 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/digibyte/digibyte/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because digibyte 22.x will not try to read these.
...
//! Convert enabled/size values to digibyte -prune setting.
...
//! Get pruning enabled value to show in GUI from digibyte -prune setting.
...
//! Get pruning size value to show in GUI from digibyte -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if digibyte.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_digibyte_unit);
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/digibyte-core/gui/issues/567).
