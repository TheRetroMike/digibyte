...
`test_digibyte` accepts the command line arguments from the boost framework.
...
test case is entered, for example. `test_digibyte` also accepts the command
line arguments accepted by `digibyted`. Use `--` to separate both types of
arguments:

```bash
test_digibyte --log_level=all --run_test=getarg_tests -- -printtoconsole=1
```
...
is happening by running `gdb ./src/test/test_digibyte` and then using the `bt` command
within gdb.
...
`test_digibyte` arguments and ensure that your ulimits are set properly (e.g. `ulimit -c
unlimited`).
...
```bash
gdb src/test/test_digibyte core
