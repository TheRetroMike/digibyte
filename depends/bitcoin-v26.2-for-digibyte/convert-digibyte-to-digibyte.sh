#!/bin/bash
# Comprehensive DigiByte to DigiByte naming conversion script for v26.2
...
find . -type f \( -name "*.cpp" -o -name "*.h" -o -name "*.c" \) | grep -v ".git/" | while read file; do
    ...
    if grep -q "Copyright.*The DigiByte Core developers" "$file" && ! grep -q "Copyright.*The DigiByte Core developers" "$file"; then
        ...
        sed -i '/Copyright.*The DigiByte Core developers/a\// Copyright (c) 2014-2025 The DigiByte Core developers' "$file"
    fi
done
