...
AC_DEFUN([DIGIBYTE_QT_FAIL],[
  if test "$digibyte_qt_want_version" = "auto" && test "$digibyte_qt_force" != "yes"; then
    if test "$digibyte_enable_qt" != "no"; then
...
AC_DEFUN([DIGIBYTE_QT_CHECK],[
  if test "$digibyte_enable_qt" != "no" && test "$digibyte_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([DIGIBYTE_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     ...
     if test "$digibyte_qt_want_version" = "yes"; then
...
AC_DEFUN([DIGIBYTE_QT_CONFIGURE],[
  ...
  if test $digibyte_enable_qt = "yes"; then
