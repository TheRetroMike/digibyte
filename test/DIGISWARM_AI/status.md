example_test.py                                        | ✓ Passed  | 2 s
feature_abortnode.py                                   | ✓ Passed  | 1 s
feature_addrman.py                                     | ✓ Passed  | 3 s
feature_anchors.py                                     | ✓ Passed  | 3 s
feature_asmap.py                                       | ✓ Passed  | 3 s
feature_bind_extra.py                                  | ✓ Passed  | 1 s
feature_bip68_sequence.py                              | ✓ Passed  | 7 s
feature_blocksdir.py                                   | ✓ Passed  | 1 s
feature_cltv.py                                        | ✓ Passed  | 1 s
feature_config_args.py                                 | ✓ Passed  | 27 s
feature_dersig.py                                      | ✓ Passed  | 1 s
feature_dirsymlinks.py                                 | ✓ Passed  | 1 s
feature_discover.py                                    | ✓ Passed  | 2 s
feature_fastprune.py                                   | ✓ Passed  | 0 s
feature_filelock.py                                    | ✓ Passed  | 1 s
feature_help.py                                        | ✓ Passed  | 0 s
feature_includeconf.py                                 | ✓ Passed  | 2 s
feature_init.py                                        | ✓ Passed  | 7 s
feature_loadblock.py                                   | ✓ Passed  | 1 s
feature_logging.py                                     | ✓ Passed  | 3 s
feature_maxtipage.py                                   | ✓ Passed  | 41 s
feature_maxuploadtarget.py                             | ✓ Passed  | 57 s
feature_minchainwork.py                                | ✓ Passed  | 10 s
feature_notifications.py                               | ✓ Passed  | 12 s
feature_nulldummy.py                                   | ✓ Passed  | 0 s
feature_posix_fs_permissions.py                        | ✓ Passed  | 0 s
feature_presegwit_node_upgrade.py                      | ✓ Passed  | 1 s
feature_proxy.py                                       | ✓ Passed  | 3 s
feature_reindex.py                                     | ✓ Passed  | 2 s
feature_reindex_readonly.py                            | ✓ Passed  | 3 s
feature_remove_pruned_files_on_startup.py              | ✓ Passed  | 2 s
feature_segwit.py --descriptors                        | ✓ Passed  | 7 s
feature_settings.py                                    | ✓ Passed  | 2 s
feature_shutdown.py                                    | ✓ Passed  | 1 s
feature_startupnotify.py                               | ✓ Passed  | 1 s
feature_uacomment.py                                   | ✓ Passed  | 2 s
interface_digibyte_cli.py --descriptors                | ✓ Passed  | 8 s
interface_digibyte_cli.py --legacy-wallet              | ✓ Passed  | 9 s
interface_http.py                                      | ✓ Passed  | 0 s
interface_rest.py                                      | ✓ Passed  | 3 s
interface_rpc.py                                       | ✓ Passed  | 2 s
mempool_accept.py                                      | ✓ Passed  | 2 s
mempool_accept_wtxid.py                                | ✓ Passed  | 16 s
mempool_datacarrier.py                                 | ✓ Passed  | 1 s
mempool_dust.py                                        | ✓ Passed  | 0 s
mempool_expiry.py                                      | ✓ Passed  | 1 s
mempool_persist.py --descriptors                       | ✓ Passed  | 13 s
mempool_resurrect.py                                   | ✓ Passed  | 0 s
mempool_spend_coinbase.py                              | ✓ Passed  | 0 s
mining_getblocktemplate_longpoll.py                    | ✓ Passed  | 67 s
p2p_add_connections.py                                 | ✓ Passed  | 7 s
p2p_addr_relay.py                                      | ✓ Passed  | 15 s
p2p_addrfetch.py                                       | ✓ Passed  | 1 s
p2p_addrv2_relay.py                                    | ✓ Passed  | 1 s
p2p_block_sync.py                                      | ✓ Passed  | 2 s
p2p_blockfilters.py                                    | ✓ Passed  | 8 s
p2p_compactblocks_blocksonly.py                        | ✓ Passed  | 1 s
p2p_compactblocks_hb.py                                | ✓ Passed  | 17 s
p2p_dandelion.py                                       | ✓ Passed  | 51 s
p2p_disconnect_ban.py                                  | ✓ Passed  | 2 s
p2p_dns_seeds.py                                       | ✓ Passed  | 26 s
p2p_eviction.py                                        | ✓ Passed  | 4 s
p2p_feefilter.py                                       | ✓ Passed  | 9 s
p2p_filter.py                                          | ✓ Passed  | 3 s
p2p_fingerprint.py                                     | ✓ Passed  | 1 s
p2p_getaddr_caching.py                                 | ✓ Passed  | 3 s
p2p_getdata.py                                         | ✓ Passed  | 1 s
p2p_i2p_ports.py                                       | ✓ Passed  | 3 s
p2p_i2p_sessions.py                                    | ✓ Passed  | 1 s
p2p_initial_headers_sync.py                            | ✓ Passed  | 1 s
p2p_invalid_locator.py                                 | ✓ Passed  | 1 s
p2p_invalid_tx.py                                      | ✓ Passed  | 7 s
p2p_message_capture.py                                 | ✓ Passed  | 1 s
p2p_net_deadlock.py                                    | ✓ Passed  | 2 s
p2p_nobloomfilter_messages.py                          | ✓ Passed  | 2 s
p2p_orphan_handling.py                                 | ✓ Passed  | 5 s
p2p_permissions.py                                     | ✓ Passed  | 13 s
p2p_ping.py                                            | ✓ Passed  | 1 s
p2p_sendtxrcncl.py                                     | ✓ Passed  | 5 s
p2p_timeouts.py                                        | ✓ Passed  | 1 s
p2p_tx_privacy.py                                      | ✓ Passed  | 2 s
p2p_unrequested_blocks.py                              | ✓ Passed  | 3 s
p2p_v2_transport.py                                    | ✓ Passed  | 5 s
rpc_bind.py --ipv4                                     | ✓ Passed  | 2 s
rpc_bind.py --ipv6                                     | ✓ Passed  | 2 s
rpc_bind.py --nonloopback                              | ✓ Passed  | 2 s
rpc_createmultisig.py                                  | ✓ Passed  | 4 s
rpc_decodescript.py                                    | ✓ Passed  | 0 s
rpc_deprecated.py                                      | ✓ Passed  | 1 s
rpc_deriveaddresses.py                                 | ✓ Passed  | 0 s
rpc_deriveaddresses.py --usecli                        | ✓ Passed  | 0 s
rpc_dumptxoutset.py                                    | ✓ Passed  | 1 s
rpc_estimatefee.py                                     | ✓ Passed  | 0 s
rpc_generate.py                                        | ✓ Passed  | 1 s
rpc_getblockfilter.py                                  | ✓ Passed  | 1 s
rpc_getblockreward.py                                  | ✓ Passed  | 1 s
rpc_getchaintips.py                                    | ✓ Passed  | 3 s
rpc_getdescriptorinfo.py                               | ✓ Passed  | 0 s
rpc_help.py                                            | ✓ Passed  | 1 s
rpc_invalid_address_message.py                         | ✓ Passed  | 0 s
rpc_mempool_info.py                                    | ✓ Passed  | 0 s
rpc_misc.py                                            | ✓ Passed  | 1 s
rpc_named_arguments.py                                 | ✓ Passed  | 0 s
rpc_net.py                                             | ✓ Passed  | 4 s
rpc_packages.py                                        | ✓ Passed  | 2 s
rpc_preciousblock.py                                   | ✓ Passed  | 2 s
rpc_rawtransaction.py --legacy-wallet                  | ✓ Passed  | 10 s
rpc_scanblocks.py                                      | ✓ Passed  | 2 s
rpc_scantxoutset.py                                    | ✓ Passed  | 2 s
rpc_setban.py                                          | ✓ Passed  | 3 s
rpc_signer.py                                          | ✓ Passed  | 1 s
rpc_signmessagewithprivkey.py                          | ✓ Passed  | 0 s
rpc_signrawtransactionwithkey.py                       | ✓ Passed  | 2 s
rpc_txoutproof.py                                      | ✓ Passed  | 2 s
rpc_uptime.py                                          | ✓ Passed  | 0 s
rpc_users.py                                           | ✓ Passed  | 6 s
rpc_validateaddress.py                                 | ✓ Passed  | 0 s
rpc_whitelist.py                                       | ✓ Passed  | 1 s
tool_wallet.py --descriptors                           | ✓ Passed  | 6 s
tool_wallet.py --legacy-wallet                         | ✓ Passed  | 7 s
wallet_abandonconflict.py --descriptors                | ✓ Passed  | 4 s
wallet_abandonconflict.py --legacy-wallet              | ✓ Passed  | 4 s
wallet_address_types.py --descriptors                  | ✓ Passed  | 12 s
wallet_address_types.py --legacy-wallet                | ✓ Passed  | 24 s
wallet_avoid_mixing_output_types.py --descriptors      | ✓ Passed  | 9 s
wallet_avoidreuse.py --descriptors                     | ✓ Passed  | 7 s
wallet_avoidreuse.py --legacy-wallet                   | ✓ Passed  | 12 s
wallet_backup.py --descriptors                         | ✓ Passed  | 27 s
wallet_backup.py --legacy-wallet                       | ✓ Passed  | 38 s
wallet_basic.py --descriptors                          | ✓ Passed  | 17 s
wallet_basic.py --legacy-wallet                        | ✓ Passed  | 16 s
wallet_blank.py --descriptors                          | ✓ Passed  | 1 s
wallet_change_address.py --descriptors                 | ✓ Passed  | 11 s
wallet_change_address.py --legacy-wallet               | ✓ Passed  | 27 s
wallet_coinbase_category.py --descriptors              | ✓ Passed  | 1 s
wallet_coinbase_category.py --legacy-wallet            | ✓ Passed  | 1 s
wallet_conflicts.py --descriptors                      | ✓ Passed  | 2 s
wallet_conflicts.py --legacy-wallet                    | ✓ Passed  | 4 s
wallet_createwallet.py --descriptors                   | ✓ Passed  | 2 s
wallet_createwallet.py --legacy-wallet                 | ✓ Passed  | 3 s
wallet_createwallet.py --usecli                        | ✓ Passed  | 3 s
wallet_crosschain.py                                   | ✓ Passed  | 1 s
wallet_descriptor.py --descriptors                     | ✓ Passed  | 2 s
wallet_disable.py                                      | ✓ Passed  | 0 s
wallet_dump.py --legacy-wallet                         | ✓ Passed  | 4 s
wallet_encryption.py --descriptors                     | ✓ Passed  | 6 s
wallet_encryption.py --legacy-wallet                   | ✓ Passed  | 6 s
wallet_fallbackfee.py --descriptors                    | ✓ Passed  | 1 s
wallet_fallbackfee.py --legacy-wallet                  | ✓ Passed  | 1 s
wallet_fast_rescan.py --descriptors                    | ✓ Passed  | 3 s
wallet_hd.py --descriptors                             | ✓ Passed  | 5 s
wallet_hd.py --legacy-wallet                           | ✓ Passed  | 6 s
wallet_implicitsegwit.py --legacy-wallet               | ✓ Passed  | 6 s
wallet_import_rescan.py --legacy-wallet                | ✓ Passed  | 17 s
wallet_import_with_label.py --legacy-wallet            | ✓ Passed  | 1 s
wallet_importmulti.py --legacy-wallet                  | ✓ Passed  | 4 s
wallet_importprunedfunds.py --descriptors              | ✓ Passed  | 1 s
wallet_importprunedfunds.py --legacy-wallet            | ✓ Passed  | 1 s
wallet_keypool.py --descriptors                        | ✓ Passed  | 2 s
wallet_keypool.py --legacy-wallet                      | ✓ Passed  | 3 s
wallet_keypool_topup.py --descriptors                  | ✓ Passed  | 6 s
wallet_keypool_topup.py --legacy-wallet                | ✓ Passed  | 10 s
wallet_labels.py --descriptors                         | ✓ Passed  | 2 s
wallet_labels.py --legacy-wallet                       | ✓ Passed  | 3 s
wallet_listdescriptors.py --descriptors                | ✓ Passed  | 1 s
wallet_listreceivedby.py --descriptors                 | ✓ Passed  | 5 s
wallet_listreceivedby.py --legacy-wallet               | ✓ Passed  | 5 s
wallet_listsinceblock.py --descriptors                 | ✓ Passed  | 7 s
wallet_listsinceblock.py --legacy-wallet               | ✓ Passed  | 10 s
wallet_listtransactions.py --descriptors               | ✓ Passed  | 18 s
wallet_listtransactions.py --legacy-wallet             | ✓ Passed  | 16 s
wallet_migration.py                                    | ✓ Passed  | 9 s
wallet_miniscript.py --descriptors                     | ✓ Passed  | 27 s
wallet_multisig_descriptor_psbt.py --descriptors       | ✓ Passed  | 1 s
wallet_multiwallet.py --descriptors                    | ✓ Passed  | 8 s
wallet_multiwallet.py --legacy-wallet                  | ✓ Passed  | 9 s
wallet_multiwallet.py --usecli                         | ✓ Passed  | 8 s
wallet_reindex.py --descriptors                        | ✓ Passed  | 1 s
wallet_reindex.py --legacy-wallet                      | ✓ Passed  | 1 s
wallet_reorgsrestore.py                                | ✓ Passed  | 3 s
wallet_rescan_unconfirmed.py --descriptors             | ✓ Passed  | 1 s
wallet_resendwallettransactions.py --legacy-wallet     | ✓ Passed  | 5 s
wallet_signmessagewithaddress.py                       | ✓ Passed  | 0 s
wallet_simulaterawtx.py --descriptors                  | ✓ Passed  | 1 s
wallet_simulaterawtx.py --legacy-wallet                | ✓ Passed  | 1 s
wallet_startup.py                                      | ✓ Passed  | 2 s
wallet_taproot.py --descriptors                        | ✓ Passed  | 23 s
wallet_timelock.py                                     | ✓ Passed  | 1 s
wallet_transactiontime_rescan.py --descriptors         | ✓ Passed  | 2 s
wallet_transactiontime_rescan.py --legacy-wallet       | ✓ Passed  | 14 s
wallet_txn_clone.py                                    | ✓ Passed  | 2 s
wallet_txn_clone.py --mineblock                        | ✓ Passed  | 3 s
wallet_txn_clone.py --segwit                           | ✓ Passed  | 1 s
wallet_txn_doublespend.py --descriptors                | ✓ Passed  | 2 s
wallet_txn_doublespend.py --legacy-wallet              | ✓ Passed  | 2 s
wallet_txn_doublespend.py --mineblock                  | ✓ Passed  | 3 s
wallet_watchonly.py --legacy-wallet                    | ✓ Passed  | 1 s
wallet_watchonly.py --usecli --legacy-wallet           | ✓ Passed  | 1 s
feature_bind_port_discover.py                          | ○ Skipped | 0 s
feature_bind_port_externalip.py                        | ○ Skipped | 0 s
feature_signet.py                                      | ○ Skipped | 0 s
feature_txindex_compatibility.py                       | ○ Skipped | 0 s
feature_unsupported_utxo_db.py                         | ○ Skipped | 0 s
interface_usdt_coinselection.py                        | ○ Skipped | 0 s
interface_usdt_mempool.py                              | ○ Skipped | 0 s
interface_usdt_net.py                                  | ○ Skipped | 0 s
interface_usdt_utxocache.py                            | ○ Skipped | 1 s
interface_usdt_validation.py                           | ○ Skipped | 0 s
interface_zmq.py                                       | ○ Skipped | 0 s
mempool_compatibility.py                               | ○ Skipped | 1 s
tool_signet_miner.py --descriptors                     | ○ Skipped | 0 s
tool_signet_miner.py --legacy-wallet                   | ○ Skipped | 0 s
wallet_backwards_compatibility.py --descriptors        | ○ Skipped | 1 s
wallet_backwards_compatibility.py --legacy-wallet      | ○ Skipped | 0 s
wallet_inactive_hdchains.py --legacy-wallet            | ○ Skipped | 0 s
wallet_upgradewallet.py --legacy-wallet                | ○ Skipped | 0 s
feature_block.py                                       | ✖ Failed  | 0 s
feature_coinstatsindex.py                              | ✖ Failed  | 2 s
feature_csv_activation.py                              | ✖ Failed  | 61 s
feature_fee_estimation.py                              | ✖ Failed  | 10 s
feature_rbf.py                                         | ✖ Failed  | 5 s
feature_segwit.py --legacy-wallet                      | ✖ Failed  | 7 s
feature_taproot.py                                     | ✖ Failed  | 2 s
feature_utxo_set_hash.py                               | ✖ Failed  | 1 s
feature_versionbits_warning.py                         | ✖ Failed  | 1 s
mempool_limit.py                                       | ✖ Failed  | 1 s
mempool_package_limits.py                              | ✖ Failed  | 1 s
mempool_package_onemore.py                             | ✖ Failed  | 0 s
mempool_packages.py                                    | ✖ Failed  | 61 s
mempool_reorg.py                                       | ✖ Failed  | 2 s
mempool_sigoplimit.py                                  | ✖ Failed  | 2 s
mempool_unbroadcast.py                                 | ✖ Failed  | 1 s
mempool_updatefromblock.py                             | ✖ Failed  | 0 s
mining_basic.py                                        | ✖ Failed  | 1 s
mining_prioritisetransaction.py                        | ✖ Failed  | 0 s
p2p_blocksonly.py                                      | ✖ Failed  | 1 s
p2p_compactblocks.py                                   | ✖ Failed  | 8 s
p2p_dos_header_tree.py                                 | ✖ Failed  | 60 s
p2p_headers_sync_with_minchainwork.py                  | ✖ Failed  | 309 s
p2p_ibd_stalling.py                                    | ✖ Failed  | 5 s
p2p_ibd_txrelay.py                                     | ✖ Failed  | 61 s
p2p_invalid_block.py                                   | ✖ Failed  | 1 s
p2p_invalid_messages.py                                | ✖ Failed  | 6 s
p2p_leak.py                                            | ✖ Failed  | 2 s
p2p_leak_tx.py                                         | ✖ Failed  | 36 s
p2p_mutated_blocks.py                                  | ✖ Failed  | 1 s
p2p_node_network_limited.py                            | ✖ Failed  | 6 s
p2p_segwit.py                                          | ✖ Failed  | 61 s
p2p_sendheaders.py                                     | ✖ Failed  | 11 s
p2p_tx_download.py                                     | ✖ Failed  | 21 s
rpc_blockchain.py                                      | ✖ Failed  | 11 s
rpc_getblockfrompeer.py                                | ✖ Failed  | 2 s
rpc_getblockstats.py                                   | ✖ Failed  | 1 s
rpc_invalidateblock.py                                 | ✖ Failed  | 1 s
rpc_psbt.py --descriptors                              | ✖ Failed  | 2 s
rpc_psbt.py --legacy-wallet                            | ✖ Failed  | 3 s
wallet_balance.py --descriptors                        | ✖ Failed  | 6 s
wallet_balance.py --legacy-wallet                      | ✖ Failed  | 5 s
wallet_blank.py --legacy-wallet                        | ✖ Failed  | 1 s
wallet_bumpfee.py --descriptors                        | ✖ Failed  | 24 s
wallet_bumpfee.py --legacy-wallet                      | ✖ Failed  | 27 s
wallet_create_tx.py --descriptors                      | ✖ Failed  | 2 s
wallet_create_tx.py --legacy-wallet                    | ✖ Failed  | 6 s
wallet_fundrawtransaction.py --descriptors             | ✖ Failed  | 9 s
wallet_fundrawtransaction.py --legacy-wallet           | ✖ Failed  | 12 s
wallet_groups.py --descriptors                         | ✖ Failed  | 6 s
wallet_groups.py --legacy-wallet                       | ✖ Failed  | 6 s
wallet_importdescriptors.py --descriptors              | ✖ Failed  | 1 s
wallet_orphanedreward.py                               | ✖ Failed  | 62 s
wallet_resendwallettransactions.py --descriptors       | ✖ Failed  | 1 s
wallet_send.py --descriptors                           | ✖ Failed  | 13 s
wallet_send.py --legacy-wallet                         | ✖ Failed  | 20 s
wallet_sendall.py --descriptors                        | ✖ Failed  | 1 s
wallet_sendall.py --legacy-wallet                      | ✖ Failed  | 1 s
wallet_signer.py --descriptors                         | ✖ Failed  | 1 s
wallet_signrawtransactionwithwallet.py --descriptors   | ✖ Failed  | 0 s
wallet_signrawtransactionwithwallet.py --legacy-wallet | ✖ Failed  | 1 s
wallet_spend_unconfirmed.py                            | ✖ Failed  | 1 s