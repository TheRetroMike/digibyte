...
class PackageRBFTest(DigiByteTestFramework):
    ...
    def assert_mempool_contents(self, expected=None):
        """Assert that the mempool contains expected transactions."""
        if expected is None:
            expected = []
        
        mempool_txids = set(self.nodes[0].getrawmempool())
        expected_txids = set([tx.txid_hex for tx in expected])
        
        if mempool_txids != expected_txids:
            self.log.error(f"Mempool contents mismatch")
            self.log.error(f"Expected: {expected_txids}")
            self.log.error(f"Actual: {mempool_txids}")
            assert False, f"Mempool contents mismatch. Expected {len(expected_txids)} txs, got {len(mempool_txids)} txs"
    ...
    def fill_mempool(self, node, tx_sync_fun):
        """Fill mempool until eviction is triggered."""
        ...
        for batch_of_txid in range(num_of_batches):
            ...
            for _ in range(tx_batch_size):
                try:
                    tx = miniwallet.create_self_transfer(fee_rate=fee)['tx']
                    for txout in txouts:
                    ...
                    # If we hit any error, continue - mempool might be full
                    break

        # Sync function is optional for this test
        if hasattr(tx_sync_fun, '__call__'):
            tx_sync_fun()
    ...
    def test_0fee_package_rbf(self):
        self.log.info("Test package RBF: 0-fee parent + high-fee child replaces parent's conflicts")
        node = self.nodes[0]
        # Reuse the same coins so that the transactions conflict with one another.
    ...
    def test_child_conflicts_parent_mempool_ancestor(self):
        self.fill_mempool(self.nodes[0], tx_sync_fun=self.no_op)
