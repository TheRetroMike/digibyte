...
def get_fee(tx_size, feerate_dgb_kvb):
    ...
    return target_fee_sat / Decimal(1e8) # Return result in  DGB
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> Tuple[dict, pathlib.Path]:
    ...
    if sys.platform == "win32":
        ...
        datadir = temp_dir / "DigiByte"
    else:
        env = dict(HOME=str(temp_dir))
        if sys.platform == "darwin":
            datadir = temp_dir / "Library/Application Support/DigiByte"
        else:
            datadir = temp_dir / ".digibyte"
    return env, datadir
...
def create_lots_of_big_transactions(mini_wallet, node, fee, tx_batch_size, txouts, utxos=None):
    ...
    for _ in range(tx_batch_size):
        ...
        # Skip fee assertion for now as DigiByte might have different testmempoolaccept format
