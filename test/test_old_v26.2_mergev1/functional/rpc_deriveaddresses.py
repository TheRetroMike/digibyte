...
class DeriveaddressesTest(DigiByteTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[0].deriveaddresses(combo_descriptor), ["svjXa74Go323G2FjCE1ajA3KRNfqUdzr6S", address, "ygzToBygUjMSsho9SpjS61HDoVWyZLHLHv"])
        ...
        # Before #26275, digibyted would crash when deriveaddresses was
        # called with derivation index 2147483647, which is the maximum
        # positive value of a signed int32, and - currently - the
        # maximum value that the deriveaddresses digibyte RPC call
        # accepts as derivation index.
        assert_equal(self.nodes[0].deriveaddresses(descsum_create("wpkh(tprv8ZgxMBicQKsPd7Uf69XL1XwhmjHopUGep8GuEiJDZmbQz6o58LninorQAfcKZWARbtRtfnLcJ5MQ2AtHcQJCCRUcMRvmDUjyEmNUWwx8UbK/1/1/*)"), [2147483647, 2147483647]), ["dgbrt1qtzs23vgzpreks5gtygwxf8tv5rldxvvs2x5kpn"])
