...
class RawTransactionsTest(DigiByteTestFramework):
    ...
    def sendrawtransaction_testmempoolaccept_tests(self):
        ...
        # Fee rate is 150.00000000 DGB/kvB (above DigiByte's 100 DGB/kvB default limit)
        tx = self.wallet.create_self_transfer(fee_rate=Decimal("150.00000000"))
        # Thus, testmempoolaccept should reject
        ...
        # and the following calls should both succeed
        testres = self.nodes[2].testmempoolaccept(rawtxs=[tx['hex']], maxfeerate='150.00000000')[0]
        assert_equal(testres['allowed'], True)
        self.nodes[2].sendrawtransaction(hexstring=tx['hex'], maxfeerate='150.00000000')
