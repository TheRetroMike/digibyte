...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, digibyted, digibyte_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        """
        Kwargs:
        ...
        self.digibyteconf = self.datadir_path / "digibyte.conf"
        ...
        self.cli = TestNodeCLI(digibyte_cli, self.datadir_path)
    ...
    PRIV_KEYS = [
            # DigiByte regtest address, privkey (from v8.22.2 reference)
            ...
            AddressKeyPair('snKJKqCfqSxuGMHH7FBMxP5xQRASiLdyEi', 'ec3GmNWcQaXrbJhxhhyyNAhVUCFNcCzJt8RtVzhmm2VyJCJPa8qg'),
    ]
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'digibyted exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
    ...
    def wait_until_stopped(self, *, timeout=DIGIBYTED_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
