...
class RawTransactionsTest(DigiByteTestFramework):
    ...
    def test_many_inputs_fee(self):
        ...
        inputs = []
        outputs = {self.nodes[0].getnewaddress():5.0,self.nodes[0].getnewaddress():3.0}
    ...
    def test_many_inputs_send(self):
        ...
        self.generate(self.nodes[1], 1)
        assert_equal(oldBalance+Decimal('72008.00000000'), self.nodes[0].getbalance()) #8.0+block reward
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 DGB each (20 DGB total)
        ...
        # Attempt to send 29 DGB from a wallet that only has 20 DGB. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 DGB payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 DGB.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 DGB)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 DGB.
    ...
    def test_input_confs_control(self):
        ...
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 DGB covered (need to fund 0.9 DGB more).
