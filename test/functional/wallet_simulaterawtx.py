...
class SimulateTxTest(DigiByteTestFramework):
    ...
    def run_test(self):
        ...
        self.generatetoaddress(node, COINBASE_MATURITY_2 + 1, w0.getnewaddress())
        assert_equal(w0.getbalance(), 72000.0)
        ...
        digibyte_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 dgb decrease, w2 sees + 5 dgb
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + digibyte_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - digibyte_fee + 4.9999       -5 - digibyte_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - digibyte_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - digibyte_fee)
        ...
        tx2 = funding["hex"]
        digibyte_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + digibyte_fee2))
