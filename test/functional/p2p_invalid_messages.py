...
class InvalidMessagesTest(DigiByteTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-whitelist=addr@127.0.0.1", "-dandelion=0", "-easypow", "-peertimeout=300"]]
    ...
    def test_invalid_pow_headers_msg(self):
        ...
        blockheader.rehash()
        # Find valid PoW for DigiByte multi-algorithm (use powHash, not hash)
        target = uint256_from_compact(blockheader.nBits)
        while blockheader.powHash > target:
        ...
        # invalidate PoW - ensure powHash > target  
        target = uint256_from_compact(blockheader.nBits)
        while blockheader.powHash <= target:
            blockheader.nNonce += 1
            blockheader.rehash()
