...
class ConfArgsTest(DigiByteTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in digibyte.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "digibyte_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'digibyte.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "digibyte_main.conf"
        util.write_config(main_conf_file_path, n=0, chain='', extra_config=f'includeconf={inc_conf_file_path}\n')
        with open(inc_conf_file_path, 'w', encoding='utf-8') as conf:
            conf.write('acceptnonstdtxn=1\n')
        self.nodes[0].assert_start_raises_init_error(extra_args=[f"-conf={main_conf_file_path}", "-allowignoredconf", "-txindex=1"], expected_msg='Error: acceptnonstdtxn is not currently supported for main chain')
    ...
    def test_config_file_log(self):
        ...
        # directory by digibyted.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a digibyte.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.digibyteconf.read_text()
        conf_path = default_datadir / "digibyte.conf"
        ...
        # (conf_path, not node.digibyteconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a digibyte.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "digibyte.conf" file which is ignored, because a '
        ...
        # the same digibyte.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/digibyte.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when digibyte.conf in the default data directory sets another datadir '
                      'and it contains a different digibyte.conf file that would be ignored')
        ...
        # digibyte.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "digibyte.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "digibyte.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/digibyte.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "digibyte.conf"
        for chain, chain_name in {("main", ""), ("test", "testnet3"), ("signet", "signet")}:
            util.write_config(conf_file, n=0, chain=chain_name, extra_config='acceptstalefeeestimates=1\n')
            self.nodes[0].assert_start_raises_init_error(extra_args=["-txindex=1"], expected_msg=f'Error: acceptstalefeeestimates is not supported on {chain} chain.')
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "digibyte.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_digibyte.conf"
