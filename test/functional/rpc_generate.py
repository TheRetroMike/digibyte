...
class RPCGenerateTest(DigiByteTestFramework):
    ...
    def test_generateblock(self):
        ...
        combo_address = 'dgbrt1qw508d6qejxtdg4y5r3zarvary0c5xw7k2875s5'  # DigiByte regtest address
        ...
        combo_address = 'sngCD65ySuZt52Kq1PcYMyo73LbVnGSHFr'  # DigiByte regtest address
        hash = self.generateblock(node, 'combo(' + combo_key + ')', [])['hash']
        block = node.getblock(hash, 2)
        assert_equal(len(block['tx']), 1)
        assert_equal(block['tx'][0]['vout'][0]['scriptPubKey']['address'], combo_address)
