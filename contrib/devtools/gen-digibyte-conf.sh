...
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/digibyte.conf}

[ ! -x "$DIGIBYTED" ] && echo "$DIGIBYTED not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($DIGIBYTED --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${DIGIBYTED}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $DIGIBYTED was built from a dirty tree.\n"
  echo -e "To safely generate a digibyte.conf file, please commit your changes to $DIGIBYTED, rebuild, then run this script again.\n"
fi

echo 'Generating example digibyte.conf file in share/examples/'
...
## Generated by contrib/devtools/gen-digibyte-conf.sh.
##
...
# parse the output from digibyted --help
