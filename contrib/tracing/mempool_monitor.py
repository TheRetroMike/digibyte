...
""" Example logging DigiByte Core mempool events using the mempool:added,
...
def main(digibyted_path):
    digibyted_with_usdts = USDT(path=str(digibyted_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    digibyted_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    digibyted_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    digibyted_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    digibyted_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[digibyted_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/digibyted")
        exit(1)
